function PsychPerformancePlot_Two(AxesHandle, Action, varargin)
...
%     mdlOpto = spline(difficulty, optoPer, splineIdx);
    ...
    switch Action
        case 'init'
            ...
            axes(AxesHandle);
            BpodSystem.GUIHandles.HM_PPP_NormalDots_Two = line(difficulty, performance, 'Marker', '.', 'LineStyle', 'none', 'Color', 'b', 'MarkerSize', 28);
            %BpodSystem.GUIHandles.HM_PPP_NormalLine = line(splineIdx, mdl, 'LineStyle', '-', 'Color', 'b', 'LineWidth', 2);
            BpodSystem.GUIHandles.HM_PPP_NormalLine_Two = line(0:1:100, mdl, 'LineStyle', '-', 'Color', 'b', 'LineWidth', 2);
            BpodSystem.GUIHandles.HM_PPP_OptoDots_Two = line(difficulty, optoPer, 'Marker', '.', 'LineStyle', 'none', 'Color', [0.9100 0.4100 0.1700], 'MarkerSize', 24);
            %BpodSystem.GUIHandles.HM_PPP_OptoLine = line(splineIdx, mdlOpto, 'LineStyle', '-', 'Color', [0.9100 0.4100 0.1700], 'LineWidth', 2);
            BpodSystem.GUIHandles.HM_PPP_OptoLine_Two = line(0:1:100, mdlOpto, 'LineStyle', '-', 'Color', [0.9100 0.4100 0.1700], 'LineWidth', 2);
            ...
            BpodSystem.GUIHandles.HM_PPP_DifLevel_Two = line([trialsDif(end) trialsDif(end)], [0 100]);
            set(gca,'box','off', 'color', 'none')
            hold(AxesHandle, 'on');
%             legend('off');
        case 'update'
            set(BpodSystem.GUIHandles.HM_PPP_NormalDots_Two, 'ydata', performance);
            set(BpodSystem.GUIHandles.HM_PPP_NormalLine_Two, 'ydata', mdl);
            set(BpodSystem.GUIHandles.HM_PPP_OptoDots_Two, 'ydata', optoPer);
            set(BpodSystem.GUIHandles.HM_PPP_OptoLine_Two, 'ydata', mdlOpto);
            set(BpodSystem.GUIHandles.HM_PPP_DifLevel_Two, 'xdata', [trialsDif(end) trialsDif(end)]);
    end
