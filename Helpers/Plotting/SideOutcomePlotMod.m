...
function SideOutcomePlotMod(AxesHandle, Action, varargin)
%% 
...
% Hernando M. Vergara 2019.Feb.13 - Modified for appearance
...
switch Action
    case 'init'
        ...
        %find the opto trials
        x=find(OptoStim);
        ystart=repelem(-2,length(x));
        ystop=repelem(3,length(x));
        ...
        BpodSystem.GUIHandles.OptoStim = plot(tx(:),ty(:));
        set(AxesHandle, 'XLim', [1 nTrialsToShow]);
        hold(AxesHandle, 'on');
        ...
        set(AxesHandle,'TickDir', 'out','YLim', [-1, 2], 'YTick', [0 1],'YTickLabel', {'Right','Left'}, 'FontSize', 6);
    ...
    case 'update'
        ...
        if ~isempty(OutcomeRecord)
            ...
            if S.RewardChangeBlock == 1
                set(AxesHandle,'TickDir', 'out','YLim', [-1, 2], 'YTick', [0 1],'YTickLabel', {'Right','Left**'}, 'FontSize', 6);
            elseif S.RewardChangeBlock == -1
                set(AxesHandle,'TickDir', 'out','YLim', [-1, 2], 'YTick', [0 1],'YTickLabel', {'Right**','Left'}, 'FontSize', 6);
            end
        end
end
