function figData=Online_NidaqPlot_align(action,...
                    subPlotTitles,Phase,figData,...
                    newData470,newData405,nidaqRaw)
global BpodSystem S
...
switch action
    case 'ini'
        ...
        event1=plot([-5 5],[0 0],'--b','LineWidth',1);
        hold off
...
%Save the figure properties
    ...
    case 'update'
        ...
        %% find times of specific states for alignment
        % get the state the data is zeroed for
        % the x data, i.e. 'newData470(:,1)', has that state time
        % subtracted already
        currentZeroState = S.Names.StateToZero{S.GUI.StateToZero};
        stateTimes = getStateTimes(BpodSystem.Data.RawEvents.Trial{end},...
                                   S.alignStates);

        set(figData.event1,'Xdata',[],'YData',[-10 10]);
        %% Update subplots
        % fetch data from current trial type subplot
        % allData (time x trials) has all trials from this trial type
        allData=get(figData.photosubplot(ss), 'UserData');
        dataSize=size(allData,2);
        % add new trace to it
        allData(:,dataSize+1)=newData470(:,3);
        % reassign it to the 'UserData' property of this subplot
        % note1: this does not include the average trace!
        % note2: this does not plot anything, just stores the data
        set(figData.photosubplot(ss),'UserData',allData);
        
        % calculate mean for different outcomes
        meanData = ones(size(allData,1),2)*nan;
        if ~isempty(S.TrialOutcomes)
            meanData(:,1)=nanmean(allData(:,S.TrialOutcomes{1}==1),2);
            meanData(:,2)=nanmean(allData(:,S.TrialOutcomes{1}==0),2);
        else
            meanData(:,1)=nanmean(allData,2);
        end
        
        % --- plotting ---
        curSubplot=figData.photosubplot(ss);
