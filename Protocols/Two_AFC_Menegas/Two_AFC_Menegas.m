...
%% think of Menegas optostim in one side
    ...
    switch S.GUI.TrainingLevel
        ...
        case {3, 4} % Auditory
            ...
            switch S.GUI.Inference
                ...
                case 6 % Implement opto-aversion task from Menegas et al, 2018 (Nao Uchida lab)
                    %override the wrong side
                    LeftPokeAction = 'LeftReward';
                    RightPokeAction = 'RightReward';
                    if (currentTrial >= S.WNOnset) % by design there will be white noise played for both left anb right trials, while having the ability to stimulate only one side (see JOP)
                            SoundType = 'WN';
                    else    
                            SoundType = 'COT'; 
                    end
            ...
            if strcmp(SoundType, 'COT') == 1 % checks if SoundType is 'COT'
                % Determine trial-specific SoundIDs and parameters 
                COTSound = CloudOfTones(sampRate, duration, rampTime, amplitude, HighFreq, LowFreq, HighPerc, LowPerc, subduration, suboverlap);
                modAmp = randomAmplitudeModulation(duration,sampRate,15,0.8);
                ModCOTSound = modAmp.*COTSound;
    ...
    sma = AddState(sma, 'Name', 'WaitForPoke', ...
        'Timer', 0, ...
        'StateChangeConditions', {'Port2In', JumpToStateAfterMiddlePoke}, ...
    ...
    sma = AddState(sma, 'Name', 'Punish', ...
        'Timer', S.PunishDelay, ...
        'StateChangeConditions', {'Tup', 'exit'}, ...
        'OutputActions', {'BNCState', OptoCondition_onPunishedPort});
    ...
    sma = AddState(sma, 'Name', 'EarlyWithdrawal', ...
     ...
     %'LargeReward opens the valves for longer and goes to drinking state.
    sma = AddState(sma, 'Name', 'LeftLargeReward', ...
        'Timer', LeftValveLongTime, ...
        'StateChangeConditions', {'Tup', 'Drinking'}, ...
        'OutputActions', {'ValveState', 1, 'BNCState', OptoCondition_onLeftPort}); 
    sma = AddState(sma, 'Name', 'RightLargeReward', ...
        'Timer', RightValveLongTime , ...
        'StateChangeConditions', {'Tup', 'Drinking'},...
        'OutputActions', {'ValveState', 4, 'BNCState', OptoCondition_onRightPort}); 
