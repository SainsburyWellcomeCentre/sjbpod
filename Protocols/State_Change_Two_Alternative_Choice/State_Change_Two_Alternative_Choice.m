...
function State_Change_Two_Alternative_Choice
...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    State_Change_Two_Alternative_Choice_GUIParameters();
...
%% Main trial loop
for currentTrial = 1:S.GUI.OriginalSoundsBlockLength
...
% if S.GUI.RecordingSide == 1
%     LowFreq = MidFreq;
% else
%     HighFreq = MidFreq;
% end
for currentTrial = S.GUI.OriginalSoundsBlockLength + 1:MaxSwitchedTrials
    ...
    switch S.GUI.TrainingLevel
        ...
        case {3, 4} % Auditory
            % Determine trial-specific SoundIDs and parameters  
            if (HighPerc> LowPerc && S.GUI.RecordingSide == 1) || (LowPerc>HighPerc && S.GUI.RecordingSide == 2)
                COTSound = CloudOfTones(sampRate, duration, rampTime, amplitude, HighFreq, LowFreq, HighPerc, LowPerc, subduration, suboverlap);
                % Modulate amplitude randomly
                modAmp = randomAmplitudeModulation(duration,sampRate,15,0.8);
                ModCOTSound = modAmp.*COTSound;
