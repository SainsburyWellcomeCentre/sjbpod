function Opto_stim_for_photometry
...
save_Trial_Settings(['opto_stim'], S);
...
ProgramPulsePalParam(1, 'Phase1Duration', 0.005); % Set output channel 1 to produce 5ms pulses
try
    ProgramPulsePalParam(1, 'InterPulseInterval', 1/S.GUI.FrequencyHz - 0.005); % Set pulse interval to produce 20Hz pulses
catch
    ProgramPulsePalParam(1, 'InterPulseInterval', 0.01);
    disp('IPI = 0.015s')
    S.GUI.OptoFreq = 1/0.015;
end
ProgramPulsePalParam(1, 'PulseTrainDuration', 0.5); % Set pulse train to last 0.5 seconds
...
%% Assemble State matrix
    ...
    sma = AddState(sma, 'Name', 'TrialStart', ...
        'Timer', 0.01, ...
        'StateChangeConditions', {'Tup', 'PhotostimOn'}, ...
        'OutputActions', {'BNCState', 2}); %outputs to BNC 2 to sync with photometry
    sma = AddState(sma, 'Name','PhotostimOn',...
        'Timer', 0.5,...s
        'StateChangeConditions',{'Tup','InterStimInterval'},...
        'OutputActions',{'BNCState', 1});%outputs to BNC 1 to drive pulse pal
