...
function Two_Alternative_Choice_CentrePortHold
...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    Two_Alternative_Choice_CentrePortHold_GUIParameters();
...
if S.GUI.VariableCueDelay == 1
    CueDelay = [S.CueDelay];
    for i = 1:MaxTrials
        CueDelay(end+1) = CueDelay(i)*1.01;
    end
end
...
for currentTrial = 1:MaxTrials
    ...
    %if CueDelay(currentTrial) < 0.1
    %    CuePlayTime = 0.2;
    %else
    %    CuePlayTime = CueDelay(currentTrial);
    %end
    %disp('CuePlayTime is:')
    %disp(CuePlayTime)
    disp('CueDelay is:') 
